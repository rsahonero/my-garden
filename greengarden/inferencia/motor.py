    ...
    def inferir(self):
        ...
        for regla in Regla.objects.all():
            if self.regla_esta_activada(regla) is False:
                if emparejar_regla(regla):
                    self._reglas_activadas.append(regla)
                    agregar_conclusion(regla.conclusion)
...
def emparejar_regla(regla):
    ...
    for premisa in regla.hecho_set.all():
        if emparejar_premisa(premisa) is False:
            return False
    return True

def emparejar_premisa(premisa):
    """
